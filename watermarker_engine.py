...
=====================================
Core image processing with PDF support and tiling fix
"""
...
def convert_pdf_to_images(pdf_path: str, dpi: int = None) -> List[Image.Image]:
    if not PDF_SUPPORT:
        logger.error("pdf2image not found")
        return []
...
def generate_filename(original_path: str, naming_mode: str, prefix: str = "", extension: str = "jpg", index: int = 1) -> str:
    ...
    except Exception: return f"output_{index:03d}.{extension}"
...
def process_image(file_path, filename, wm_obj, resize_cfg, out_fmt, quality) -> Tuple[bytes, Dict]:
    with Image.open(file_path) as img_temp:
        img = ImageOps.exif_transpose(img_temp)
        exif_data = img.info.get('exif')
    ...
    if sf != 1.0: img = img.resize((new_w, new_h), Image.Resampling.LANCZOS)
    if wm_obj: img = _apply_watermark(img, wm_obj, resize_cfg)
    
    res_bytes = _export_image(img, out_fmt, quality, exif_data)
    stats = {"filename": filename, "orig_res": f"{orig_w}x{orig_h}", "new_res": f"{new_w}x{new_h}", "orig_size": orig_size, "new_size": len(res_bytes), "scale_factor": f"{sf:.2f}x"}
    return res_bytes, stats
...
def _apply_watermark(img, wm_obj, cfg):
    ...
    if angle != 0: wm_res = wm_res.rotate(angle, expand=True, resample=Image.BICUBIC)
    if pos == 'tiled': return _apply_tiled_watermark(img, wm_res, cfg)
    return _apply_corner_watermark(img, wm_res, pos, cfg)

def _apply_tiled_watermark(img, wm, cfg):
